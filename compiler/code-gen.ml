...
let convert_list_to_set lst =
  let append_uniq a b = if List.mem b a then a else b :: a in
   List.rev (List.fold_left append_uniq [] lst) ;;
...
let expand_sexprs_list lst = 
  ...
  (convert_list_to_set (List.concat addition));;
    ...
    String.concat ", "ascii_strs in
      ...
      match exps with 
        ...
        | Sexpr(String(s))::con -> let f =const_tabl@[
                              (Sexpr(String(s)),
                              ((increase_offset(s),"MAKE_LITERAL_STRING " ^ string_to_ascii_list s ^ "\n")))] in 
                              (check_exp con f)
...
let make_test exp = 
  ...
  let _to_set = convert_list_to_set _scanned in 
...
let test exp = 
  let scanned = List.append primitive (List.concat (List.map (fun x -> scan_ast_var x) exp)) in 
  let to_set = convert_list_to_set scanned in 
...
let count_label = ref 0;;
    ...
    let increase_count ()= 
    let inc ()= count_label:= !count_label + 1 in
  begin inc (); !count_label end;;
  ...
  let rec gen const_table fvars_table e depth = 
    match e with 
      ...
      | Set'((VarParam(_, minor)),exp) ->  (gen const_table fvars_table exp depth)
                                               ...
                                               "\n" ^
                                                   "mov qword [rbp + 8 * (4+" ^ (string_of_int minor) ^ ")], rax\n"
                                                    ^ "mov rax, SOB_VOID_ADDRESS\n" 
      | If'(tst,thn,els) -> let _id = string_of_int (increase_count ()) in
      ...
      | LambdaSimple'(paramtesrs,body) -> 
        let _id = string_of_int (increase_count ()) in
      ...
      | ApplicTP'(proc,args) -> 
        ...
        (gen const_table fvars_table proc depth) ^ "\n" ^ 
         "CLOSURE_ENV rbx, rax\n 
          push rbx\n
          push qword[rbp + (8 * 1)]
          SHIFT_FRAME " ^ string_of_int (3+(List.length args)) ^ "\n"^
          "CLOSURE_CODE rbx, rax\n
          jmp rbx\n"
           
         | LambdaOpt'(paramtesrs,opt,body) -> 
        let _id = (increase_count ()) in
        let lcode = (string_of_int _id) in 
        let lcont = (string_of_int _id) in
        let d =  (string_of_int depth) in  
        "EXT_ENV " ^ d ^ "\n" ^
        "mov rbx, rax\n"^
        "MAKE_CLOSURE(rax, rbx, "  ^ "Lcode"^lcode ^ ")\n"^
        "jmp Lcont" ^ lcont ^ "\n" ^
        "Lcode" ^ lcode ^ ":\n" ^
...
let gen_test exp = 
  let fvars_t = test exp in
  let const_t = make_test exp in 
  let last = List.map (fun e -> gen const_t fvars_t e 0) exp in
  print_string (List.hd last);;
